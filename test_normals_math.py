...
def run():
    with sync_playwright() as p:
        ...
        # Inject script to evaluate dot products specifically for neck and pegbox
        page.evaluate("""
            () => {
                ...
                const checkMeshes = { 'neck': neckMesh, 'pegbox': pegboxMesh };
                ...
                for (const [partName, mesh] of Object.entries(checkMeshes)) {
                    ...
                    console.log(`${partName}: Total vertices: ${total}, Culled: ${culled}. Dot range: [${minDot.toFixed(2)}, ${maxDot.toFixed(2)}]`);
                }
            }
        """)
        browser.close()
